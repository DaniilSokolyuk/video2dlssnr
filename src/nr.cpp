...
ImageF RunSrPass(GpuContext& gpu, NgxSession& ngx, std::vector<std::unique_ptr<SrStage>>& cache,
                 const ImageF& in, unsigned tW, unsigned tH, bool hdr) {
    const unsigned rW = static_cast<unsigned>(in.w), rH = static_cast<unsigned>(in.h);
    SrStage* st = nullptr;
    for (auto& s : cache)
        if (s->rW == rW && s->rH == rH && s->tW == tW && s->tH == tH) {
            st = s.get();
            break;
        }
...
ImageF UpscaleTo(GpuContext& gpu, NgxSession& ngx, std::vector<std::unique_ptr<SrStage>>& cache,
    ...
    while ((static_cast<unsigned>(cur.w) < tW || static_cast<unsigned>(cur.h) < tH) &&
        ...
        try {
            cur = RunSrPass(gpu, ngx, cache, cur, nW, nH, hdr);
...
SrStage* GetSrStage(GpuContext& gpu, NgxSession& ngx, std::vector<std::unique_ptr<SrStage>>& cache,
                    unsigned rW, unsigned rH, unsigned tW, unsigned tH, bool hdr) {
    for (auto& s : cache)
        if (s->rW == rW && s->rH == rH && s->tW == tW && s->tH == tH) return s.get();
    auto s = std::make_unique<SrStage>();
    s->rW = rW;
    s->rH = rH;
    s->tW = tW;
...
int RunNeuralRendering(const std::string& dllDir, int adapter, const std::string& inPath,
                       ...
                       bool writeDiff, bool writeOrig, bool verbose) {
    ...
    for (const std::string& imgPath : images) {
        ...
        // NR posts.
        t0 = Clock::now();
        ImageF work = isUpscale ? UpscaleTo(gpu, ngx, srCache, img, tW, tH, hdr) : img;
...
int RunNeuralRenderingVideo(const std::string& dllDir, int adapter, unsigned inW, unsigned inH,
                            ...
                            bool motionVis, int motionEngine, bool verbose) {
    SetLogToStderr(true);
    ...
    SrStage* sst = singlePass ? GetSrStage(gpu, ngx, srCache, inW, inH, tW, tH, hdr) : nullptr;
    ...
    std::thread reader([&] {
        ...
        // frames means a cut, so NR history is reset rather than dragged across it (Merserk uses
        // the same 0.24 threshold).
    ...
    while (qIn.pop(vf)) {
        ...
        if (singlePass) {
            ...
            ImageF up = isUpscale ? UpscaleTo(gpu, ngx, srCache, img, tW, tH, hdr) : img;
